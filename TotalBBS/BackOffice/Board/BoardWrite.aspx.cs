    ...
    public partial class BoardWrite : AdminBasePage
    {
        ...
        protected void lbtnCancel_Click(object sender, EventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                //ErrorLogger_Tx_Dac.GetErrorLogger_Tx_Dac().ED_CO_INSERT_ERROR_LOGGER_INFO(ex, admin.MemberId, admin.MemberNm);
                #endregion
            }
        }
        #endregion
            ...
            try
            {
                ...
                if (ds != null)
                {
                    ...
                    foreach (DataRow dr in dt1.Rows)
                    {
                        if (iData == 0)
                        {
                            OldFileName = dr["OldFileName"].ToString();
                            NewFileName = dr["NewFileName"].ToString();
                            FileUploadPath = dr["FilePath"].ToString();
                        }
                        iData = iData + 1;
                        FileBean AFBean = new FileBean();
                        AFBean.strRealFileName = dr["OldFileName"].ToString();      // 파일 원본 이름
                        AFBean.strFileName = dr["NewFileName"].ToString();          // 파일 업로드후 생성된 새이름
                        AFBean.strFilePath = dr["FilePath"].ToString();             // 파일 업로드 경로 디비에 넣지 않는경우 /FileUpload/  + FileUploadPath경로 수동 입력 해도됨
                ...
                if (!ChkBoxListData.Equals("0"))
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        LocalFileControls.FileDelete_New(FileUploadPath, dr["NewFileName"].ToString());
                    }
                }
                return ReturnValue;
            }
